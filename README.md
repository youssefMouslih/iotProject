...
Create a `.env` file in the project root. See the provided `.env` file for all available options:

```bash
# Location (Weather API calls)
FIXED_LAT=33.9887277
FIXED_LON=-6.8572714

# Weather (Optional - uses Open-Meteo fallback if not set)
OPENWEATHER_API_KEY=your_openweather_api_key
...
BREVO_SENDER_NAME=IoT Alerts

# SMS (Infobip)
INFOBIP_API_KEY=your_infobip_api_key
INFOBIP_BASE_URL=https://your-subdomain.api.infobip.com
INFOBIP_MESSAGES_URL=https://your-subdomain.api.infobip.com/sms/3/messages
INFOBIP_SENDER=InfoSMS
...
**⚠️ Security Note:** Never commit `.env` file with real API keys to version control. The `.env` file is already in `.gitignore`. Keep production secrets in environment variables or a secrets manager.
